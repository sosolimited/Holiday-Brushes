...
void draw() {
  ...
  if (mousePressed && erase != true) {
    ...
    //draw the confetti
    float confettiLength = int(random(1,3))*10;
    ...
    if (chance > 6) {
      ...
      pushMatrix();
        ...
        if (digit ==0){
          drawConfettiSticks(hotpink, hotpinkB, xpos, ypos, confettiLength);
        }
        else if (digit ==1){
          drawConfettiSticks(medturq, medturqB, xpos, ypos, confettiLength);
        }
        else if (digit ==2){
          drawConfettiSticks(tangerine, tangerineB, xpos, ypos, confettiLength);
        }
        else if (digit ==3){
          drawConfettiSticks(purple, purpleB, xpos, ypos, confettiLength);
        }
      popMatrix();
    }
...
void drawConfettiSticks(color color1, color color2, float passXpos, float passYpos, float passConfettiLength){
  stroke(color2);
  line(1 + passXpos, passYpos, passXpos + passConfettiLength, 1 + passYpos);
  stroke(color1);
  line(passXpos, passYpos, passXpos + passConfettiLength, passYpos);
};
